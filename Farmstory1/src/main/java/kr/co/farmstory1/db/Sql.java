...
public class Sql {
	...
	public static final String SELECT_LATEST = "SELECT `seq`,`title`,`rdate` FROM `Jboard_article` WHERE `cate`=? AND `parent`=0 ORDER BY `seq` DESC LIMIT 5;"; // 카테고리에 해당되는 글의 최신글(정렬) 5개제한
	
																								// 댓글을 빼야하므로 조건에 넣어주어야한다 parent=0
